...
- `create_task`: `{"project_id": 1, "name": "xxx", "assignee_name": "xxx", "phase": "concept", "priority": "high", "end_date": "2026-05-15"}`
...
def confirm_all(mid):
    ...
    for c in pending:
        try:
            ...
            nv = {}
        try:
            _execute_change(conn, c["change_type"], c["target_type"], c["target_id"], nv)
            conn.execute(
                "UPDATE meeting_changes SET status='confirmed', confirmed_by=?, confirmed_at=? WHERE id=?",
                (g.user["id"], now, c["id"]),
            )
            confirmed += 1
        except Exception as e:
...
def _execute_change(conn, change_type, target_type, target_id, new_value):
    ...
    if change_type == "create_project":
        ...
        start_date = new_value.get("start_date") or date.today().isoformat()
        collab_ids = ""
