...
from agent.knowledge import build_knowledge, visible_retriever
...
- 问「XX 附件/文档/提案里写了什么 / 那份 PDF 讲了啥 / 某某方案里提到什么」（想知道附件正文内容）→ 直接检索知识库（附件正文已建索引），基于检索到的原文回答，检索不到就说没有。
- 问「XX 任务/项目当初为什么这么定 / 老板选了哪个方案 / 那件事后来谁跟进 / 讨论里提到过什么」（想知道【历史讨论、决策、描述】等人写下的叙事）→ 也检索知识库：任务/项目的描述、子任务、评论、附件名已聚合成「档案」建索引，基于检索到的原文回答，检索不到就说没有。这类信息结构化工具查不到，别靠 get_task_detail 一个个翻。

【检索 vs 工具的边界】知识库存的是【人写下的文字】的快照（描述/讨论/决策/附件正文），适合回答「讲了什么、为什么、当初怎么定的」这类叙事问题；但它不是实时的。凡是问【当前状态】——到哪一步了、进度多少、几个逾期、本周要交什么、谁负责——一律走上面的只读工具（get_my_tasks / get_project_status / get_task_detail 等）拿实时事实，不要用知识库里的快照答当前进度。
...
def _build_knowledge(run_context=None):
    """按身份挂载知识库（附件正文语义检索）：

    - 未绑定：返回 None —— 不挂 knowledge，连 search_knowledge 工具都不注入，零 token 浪费
      （与 _build_tools 对未绑定用户的处理一致）。
    - 已绑定：返回 knowledge 单例 —— agent 自动获得 search_knowledge 工具。可见性不在这里管，
      而由 knowledge_retriever=visible_retriever 按「当前对话者可见的 project_id」过滤（双保险：
      这里已绑定才挂 + retriever 层再按项目过滤）。

    run_context 默认 None（视作未绑定），原因同 _build_instructions：兼容埋点库的无参内省调用。
    """
    if _identity_of(run_context).get("bound"):
...
def get_agent() -> Agent:
    ...
    if _agent is None:
        ...
        _agent = Agent(
            ...
            tools=_build_tools,
            # knowledge 走 callable 工厂：只给已绑定用户挂（未绑定返回 None）。
            # knowledge_retriever 固定为 visible_retriever——它内部按 run_context 身份做可见性过滤，
            # 本身就是「按身份」的，无需再包一层工厂。
    ...
    async def _smoke() -> None:
        ...
        await _run(
            ...
            [
                ...
                "「飞叶学校户外雨棚捐赠」这个项目到哪了？",
                "我有什么新消息吗？",
                "D11 浓缩咖啡液的包装提案里提了哪些盒型？",  # 附件正文检索：应基于 knowledge 检索到的 PDF 原文作答
